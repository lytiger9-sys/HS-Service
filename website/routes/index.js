...
export function createIndexRouter(context) {
  ...
  const buildGuideData = () => {
    ...
    const dashboardFeatureDetails = [
      ...
      ["logs", "로그", "서버 변경·메시지·채널·역할 지급·제재 등 서버 로그를 통합 채널과 항목별로 설정합니다."],
            ["partner", "파트너 자동화", "파트너 신청·승인 채널, 파트너 채널, 홍보 웹훅과 상단배너 라이선스를 관리합니다."],
    ...
    const planMeta = {
      ...
      standard: {
        ...
        guideDescription: "Standard는 서버 운영자가 반복적으로 처리하던 업무를 자동화하기 위한 플랜입니다. 환영 메시지, 티켓, 통합 로그와 닉네임 관리로 신규 유저 응대와 문의·관리 업무를 체계화할 수 있습니다."
      },
      pro: {
        price: "가격 미정",
        audience: "서버의 모든 기능과 확장 운영을 원하는 분들을 위한 플랜",
        subtitle: "보안·이벤트·파트너·상점·구매후기를 통합하는 최상위 단계",
        guideDescription: "Pro는 Discord 서버 운영에 필요한 모든 기능을 하나의 운영 플랫폼으로 통합하는 플랜입니다. 레이드 방지, 초대 링크 제어, 제재 관련 관리와 이벤트 기능에 더해 파트너 웹훅, 홍보 메시지 자동 전송 및 상단 배너 운영을 지원합니다. 또한 상점 임베드, 상품, 캐시 지급, 출석 보상과 생일 보상으로 유저 활동을 보상하고 커뮤니티 운영을 확장할 수 있습니다. 구매로그를 남긴 뒤 대상 유저에게 DM으로 후기를 요청하고, 받은 후기를 지정 채널에 자동 게시할 수도 있습니다."
      }
    };
    const cardFeatureLabels = {
      free: [
        "기본 관리자 도구를 바로 사용",
        "역할 관리를 손쉽게 설정",
        "임시 음성 채널을 생성",
  ...
  router.get("/guide/:planSlug", async (req, res, next) => {
    try {
      ...
      const resolvedPlanId = getPlanDefinition(license.plan || "free").id;
      const serverPlan = guide.plans.find((item) => item.slug === resolvedPlanId) || guide.plans[0];
