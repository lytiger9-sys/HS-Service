...
export const boostOn = {
  data: new SlashCommandBuilder().setName("부스트로그켜기").setDescription("현재 채널에 서버 부스트 로그를 켭니다."),
...
export const boostOff = {
  data: new SlashCommandBuilder().setName("부스트로그끄기").setDescription("서버 부스트 로그를 끕니다."),
  async execute(interaction, context) {
    if (!canManage(interaction)) return interaction.reply({ content: "서버 관리 권한이 필요합니다.", ephemeral: true });
